...
def get_diarization_sequences(limit=10, filters=None, max_sequence_length=30, worker_id=None) -> Iterator[DiarizationSequence]:
    ...
    # Query for chunks that haven't been diarized yet
    # Match chunks where diarized_at is missing or null, and processing_by is missing or null
    base_filters = {
        '$and': [
            {
                '$or': [
...
def diarize_sequence(sequence: DiarizationSequence, worker_id: str):
    ...
    try:
        if not claim_sequence(sequence, worker_id):
            ...
            return {"status": "skipped", "chunks": 0, "duration": 0, "segments": 0}
        ...
        if not segments:
            ...
            mark_as_diarized(sequence)
            end_time = time.time()
            duration = end_time - start_time
            tqdm.write(f'{timestamp}  {chunks_count:3d} chunks  {original_id}  {duration:5.2f}s  no_segments')
            return {"status": "no_segments", "chunks": chunks_count, "duration": duration, "segments": 0}
        ...
        return {"status": "diarized", "chunks": chunks_count, "duration": duration, "segments": saved_segments}
        ...
        return {"status": "error", "chunks": 0, "duration": end_time - start_time, "segments": 0}
...
def mark_as_diarized(seq: DiarizationSequence):
    """
...
def process_diarization_sequences(limit=None, max_workers=1, worker_id=None):
    ...
    tqdm.write("\n" + "=" * 80)
    tqdm.write(f"Completed: {processed_count} sequences, {total_chunks} chunks, {total_segments} segments")
